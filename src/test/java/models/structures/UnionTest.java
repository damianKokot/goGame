...
public class UnionTest {
   ...
   @Test
   public void shouldCreateOneFatherSet() {
      int testObjectsCount = 100;
      Union[] unions = UnionTestObject.getUnions(testObjectsCount);
      int[] connections = UnionTestObject.createRandomConnections(testObjectsCount);

      for(int i = 1; i < 100; ++i) {
         unions[i].union(unions[connections[i]]);
      }

      Assert.assertEquals(testObjectsCount - 1, UnionTestObject.countChildes(unions[0]));
   }
